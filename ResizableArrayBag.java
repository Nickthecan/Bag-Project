...
public final class ResizableArrayBag<T> implements BagInterface<T> {
   ...
   /** The union of two collections consists of their contents combined into a new collection
	...
	public BagInterface<T> union(BagInterface<T> insertedBag) {
      //makes sure the inserted bag does not go over the default capacity
      checkCapacity(DEFAULT_CAPACITY);
      //makes sure the object is initialized
      checkintegrity();
      //create an Array from the inserted Bag
      T[] Bag = insertedBag.toArray();
      //create a new BagInterface<T> result Bag
      BagInterface<T> resultBag = new ResizableArrayBag<>();
      //add the elements from bag 1 into the resultant bag
      for (int i = 0; i < getCurrentSize(); i++)
         resultBag.add(bag[i]);
      //add the elements from bag 2 into the resultant bag
	...
	public BagInterface<T> intersection(BagInterface<T> insertedBag) {
      ...
      BagInterface<T> copyInsertedBag = new ResizableArrayBag<>();
      ...
      for (int i = 0; i < bag.length; i++) {
         //make sure if the element is null, to jump outside the for loop because the bag has no more elements to compare
         if (bag[i] == null)
            break;
         ...
         if (copyInsertedBag.contains(bag[i])) {
            resultBag.add(bag[i]);
            copyInsertedBag.remove(bag[i]);
         }
      }
      //return resultBag
      return resultBag;
	...
	public BagInterface<T> difference(BagInterface<T> insertedBag) {
      ...
      for (int i = 0; i < insertedBag.getCurrentSize(); i++)
         copyInsertedBag.add(insert[i]);
      //for loop in order to run through bag and compare to see if the inserted bag does not contain each element
      for (int i = 0; i < bag.length; i++) {
         //make sure if the element is null, to jump outside the for loop because the bag has no more elements to compare
         if (bag[i] == null)
            break;
         //if no elements are contained in the inserted bag, add it to the result bag
         if (!copyInsertedBag.contains(bag[i]))
            resultBag.add(bag[i]);
         //otherwise, remove the element from the inserted bag to prevent duplicates
         else
            copyInsertedBag.remove(bag[i]);
      }
      //return resultBag
